...
class DataManager: ObservableObject {
    ...
    private func childrenCacheKey(for userId: String) -> String {
        return "cached_children_\(userId)"
    }
    ...
    func cacheChildren(_ children: [Child], userId: String? = nil) {
        let userId = userId ?? getCurrentUserId()
        if let encoded = try? JSONEncoder().encode(children) {
            UserDefaults.standard.set(encoded, forKey: childrenCacheKey(for: userId))
        }
    }
    
    func getCachedChildren(userId: String? = nil) -> [Child]? {
        let userId = userId ?? getCurrentUserId()
        guard let data = UserDefaults.standard.data(forKey: childrenCacheKey(for: userId)) else { return nil }
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
    ...
    func clearCache(userId: String? = nil) {
        ...
        UserDefaults.standard.removeObject(forKey: childrenCacheKey(for: userId))
