...
router.get('/dashboard', asyncHandler(async (req, res) => {
  ...
  if (!parent) {
    throw createError('Parent not found', 404, 'PARENT_NOT_FOUND');
  }
  ...
  if (childrenError) {
    ...
    throw createError('Failed to fetch children', 500, 'FETCH_CHILDREN_FAILED');
  }
...
// @access  Private (Parent only)
router.get('/children/:childId', asyncHandler(async (req, res) => {
  ...
  if (relationError || !parentChild) {
    throw createError('Child not found or not linked to this parent', 403, 'CHILD_NOT_LINKED');
  }
  
  // Get child user
  const child = await User.findById(childId);
  if (!child) {
    throw createError('Child not found', 404, 'CHILD_NOT_FOUND');
  }
...
// @access  Private (Parent only)
router.get('/children/:childId/activity', asyncHandler(async (req, res) => {
  const { childId } = req.params;
  const { getSupabase } = require('../config/database');
  const supabase = getSupabase();
...
router.post('/children/create', [
  ...
  if (existingUser) {
    throw createError('Email already in use', 400, 'EMAIL_IN_USE');
  }
  ...
  if (authError) {
    ...
    throw createError(authError.message || 'Failed to create user', 400, 'AUTH_ERROR');
  }
  ...
  if (relationError) {
    ...
    throw createError('Failed to link child to parent', 500, 'RELATION_CREATION_FAILED');
  }
...
router.get('/connections', asyncHandler(async (req, res) => {
  ...
  if (connectionsError) {
    ...
    throw createError('Failed to fetch connections', 500, 'FETCH_CONNECTIONS_FAILED');
  }
...
// @access  Private (Parent only)
router.post('/children/:childId/login-code', asyncHandler(async (req, res) => {
  const { childId } = req.params;
  const { getSupabase } = require('../config/database');
  const supabase = getSupabase();
  ...
  if (codeError) {
    ...
    throw createError('Failed to generate login code', 500, 'CODE_GENERATION_FAILED');
  }
...
router.post('/children/link-by-email', [
  ...
  if (!child) {
    throw createError('Child account not found with this email', 404, 'CHILD_NOT_FOUND');
  }
  
  if (child.userType !== 'kid') {
    throw createError('This email belongs to a parent account, not a child account', 400, 'INVALID_USER_TYPE');
  }
  ...
  if (existingRelation && !checkError) {
    throw createError('Child is already linked to your account', 400, 'ALREADY_LINKED');
  }
  ...
  if (relationError) {
    ...
    throw createError('Failed to link child to parent', 500, 'LINK_FAILED');
  }
